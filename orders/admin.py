...
class OrderAdmin(admin.ModelAdmin):
    def order_detail(obj):
        ...
        return format_html('<a href="{0}">View</a>',reverse('orders:admin_order_detail', args=[obj.id]))

    order_detail.allow_tags = True

    def order_pdf(obj):
        return format_html('<a href="{0}">PDF</a>',reverse('orders:admin_order_pdf', args=[obj.id]))
